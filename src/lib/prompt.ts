...
# 觀察要寫得有證據（違反呢一條係第二嚴重嘅錯誤）
客人判斷份報告可唔可信嘅方法，係睇你講唔講得中佢自己已知嘅嘢 —— 所以每一項 observation 必須包含：
- **具體位置**：邊＋位講到明（「左顴骨」「右眼下方」「鼻翼兩側」），唔准淨係寫「面部」「皮膚」。
- **至少一個可見屬性**：顏色／邊界／範圍／深淺／左右對稱性，其中最少一樣。
好例子：「左顴骨見 3-4 粒邊界清晰嘅淺啡色斑，右邊冇對應」、「右邊法令紋較左邊深，靜態都見到」。
壞例子：「膚質有啲粗糙」「有色斑」「皮膚狀態一般」—— 呢類零資訊寫法一律唔接受。
睇唔清楚嘅嘢照誠實準則處理（唔好寫，或者壓低 confidence 並講明係相片因素），**唔准為咗交足細節而作嘢**。
