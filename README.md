...
手機自拍 → AI 分析皮膚狀況同面部輪廓 → 配對診所實際提供嘅療程。
...
> 直接複製俾人睇嘅報告。
>
> **最快上線（唔使 terminal）**：睇 **[DEPLOY.md](./DEPLOY.md)** 最上面嗰個
> Deploy 掣，撳完填幾格就有條公開網址。個 QR 只有喺公開網址之下先掃得到。
...
### 「最平但最準」實際點揀：`npm run compare`

```bash
npm run compare -- --dir eval/photos --yes
```

`npm run eval` 要醫生逐張相標註（幾十個鐘醫生時間）。`npm run compare` 係中間方案：
**用最強嘅模型做參照標準**，睇下平嘅模型同佢差幾遠。唔使標註、唔使等醫生。
三十張相、約 HK$30 就有答案。

答唔到「有幾準」，但答到**「平嗰個蝕咗幾多」**—— 而後者先係你而家要做嘅決定。

| 特徵一致率 | 點做 |
|---|---|
| 85%+ | 平嗰個夠用，慳到嘅錢係淨賺 |
| 70–85% | 客人免費版用平嘅，到診後用貴嘅再跑一次 |
| <70% | 唔好慳，差異已經影響到客人見到嘅建議 |

**但最重要嗰個數字唔係一致率，係紅旗召回率。** 平模型平均準確度差少少通常唔緊要
（醫生面診會執返），但佢漏咗一粒可疑嘅痣，就係完全唔同性質嘅事。所以個 script
會單獨報：平模型有冇漏咗參照模型捉到嘅紅旗。**漏報一個都唔應該接受；多報係可以接受嘅** ——
客人白行一趟皮膚科，同漏咗一個可能係皮膚癌嘅病灶，唔係同一個量級。

> ⚠️ 如果你批相入面一個紅旗都冇，個 script 會明講「呢個測試驗證唔到漏報」——
> 唔會扮咗測過。要測到就要特登加幾張有可疑痣嘅相。
...
**隱私**：相片經 API route 直接轉發俾 Anthropic，唔會寫入伺服器磁碟、唔會入資料庫 —— 呢點由始至終冇變。另外有一個**分開嘅、預設唔剔嘅**自願選項：客人可以留電話儲存當次嘅 8 大範疇評分同觀察項目編號（`src/lib/visits.ts`，唔包括相片、唔包括文字描述），保留 24 個月，職員可以喺 `/records` 應要求刪除。呢個安排嘅《個人資料（私隱）條例》處理（同意文字、保留期、刪除權）已經內置。
...
> ⚠️ `Consent.tsx` 嘅「唔會儲存」講嘅係**相片** —— 呢句到而家都係真確嘅：評分儲存（`SaveRecordCard.tsx`）係另一個分開嘅同意，只儲電話＋分數＋項目編號，唔掂相片。如果日後真係要儲**相片**（前後對比），呢句先至要改，而且要成個同意書重寫。
...
- ⚠️ 同樣係記憶體儲存，重啟清零。要長期趨勢就實作 `FunnelStore` 駁去資料庫（做法可以照抄 `visits.ts` + `visits-d1.ts` 嗰套：介面＋記憶體實作＋D1 實作）
...
- 客人紀錄（`/records`）儲評分數字，唔儲相片 —— 所以有分數趨勢，但冇「前後對比相」。要相片對比就要另一輪私隱處理（見上面隱私一節）
- 客人紀錄未接駁 D1 之前係記憶體實作（重啟清零）—— 接駁步驟喺 `DEPLOY.md`
- 速率限制同漏斗統計都係**記憶體**實作：重啟清零，serverless 多 instance 唔會加埋一齊。兩者都留咗介面（`RateLimitStore` / `FunnelStore`），要準確就換 Redis / 資料庫
- 職員頁面（`/pro`、`/records` 等）用單一共享密碼（`STAFF_TOKEN`），冇逐人帳戶、冇審計紀錄 —— 對幾個同事嘅診所係合理取捨
...
|---|---|---|
| `/` | **客人** | 公開 |
| `/embed` | 客人（嵌入官網用，暫時唔使） | 公開，但要喺允許清單先載入到 |
| `/pro` | 診所內部 | **要** `STAFF_TOKEN` |
| `/share` | 診所內部：印 QR 海報、copy 宣傳文案 | **要** `STAFF_TOKEN` |
| `/embed/setup` | 診所內部：嵌入設定 | **要** `STAFF_TOKEN` |

### 職員頁面保護（`src/lib/staff-auth.ts`）

一部署上網，所有路徑都係公開嘅。客人版公開係應該嘅，但 `/pro` 唔係 ——
佢睇到全部療程詳情、分階段方案、每次分析嘅實際成本，而且**每撳一次都係你俾錢**。

用最簡單嘅共享密碼，唔做帳戶系統：診所得幾個同事，一個登入系統嘅維護成本
（改密碼、重設、鎖帳戶）遠高於佢解決嘅問題。

- 開 `https://…/pro?k=你個密碼` 一次 → 種 cookie，30 日唔使再輸
- 種完 cookie 即刻將 `?k=` 由網址剝走 —— 否則密碼會留喺瀏覽器歷史、書籤、
  同分享出去嘅連結入面
- **未設 `STAFF_TOKEN` + 正式環境 → 直接 403**，唔會靜靜雞放行。測試模式照開，方便試

> ⚠️ 由加入客人紀錄（`/records`）開始，呢個 token 亦係保護個人資料
> （電話號碼＋皮膚評分）嘅唯一屏障，唔再只係擋「陌生人用你嘅 API 額度」。
> `STAFF_TOKEN` 要夠長夠隨機，懷疑洩露就即刻換。

### 派俾客人（`/share`）

一個部署咗但冇人搵到嘅工具等於冇做過。唔改官網嘅話，實際得三條路，
`/share` 將三條都變成撳一下就用得：

1. **QR 海報** —— 撳「列印」出一張 A4，擺接待處 / 等候區 / 診症室。
   客人一路等一路掃，做完分析就已經帶住問題入去見醫生，面診質素高好多
2. **WhatsApp 文案** —— 已經寫好。最有用嘅時機唔係新客，係**舊客跟進**：
   做完療程一個月 send 俾佢，佢自己會發現下一個想改善嘅地方
3. **IG / 小紅書貼文** —— 已經寫好，連結擺 profile

QR 由 `/api/qr` 產生（SVG，印到幾大都清）。嗰個 endpoint **刻意唔收
「要編碼咩內容」呢個參數**，只收本站路徑 —— 否則等於免費借咗你個網域
俾人做釣魚 QR：人哋派街招，個 QR 由你個網域出，指去佢個假網站，出事嘅係你。
